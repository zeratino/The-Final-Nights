	...
	if (iskindred(living_owner))
		...
		if (COOLDOWN_FINISHED(kindred_species, torpor_timer) && (vampire.bloodpool > 0))
			...
			if (vampire.bloodpool > 0)
				...
				to_chat(usr, "<span class='purple'><i>The time to re-awaken depends on your [(vampire.morality_path.score > 5) ? "high" : "low"] [vampire.client.prefs.is_enlightened ? "Enlightenment" : "Humanity"] rating of [vampire.morality_path.score].</i></span>")
			else

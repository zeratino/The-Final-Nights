		...
		if("input")
			...
			switch(href_list["preference"])
				...
				*/
				...
				if("ooc_notes")
					var/new_ooc_notes = tgui_input_text(user, "Choose your character's OOC notes:", "Character Preference", ooc_notes, MAX_MESSAGE_LEN, TRUE, TRUE)
					if(!length(new_ooc_notes))
						return
					ooc_notes = new_ooc_notes

				if("flavor_text")
					var/new_flavor = tgui_input_text(user, "Choose your character's flavor text:", "Character Preference", flavor_text, MAX_FLAVOR_LEN, TRUE, TRUE)

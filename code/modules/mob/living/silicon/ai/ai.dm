			...
			for (var/alarm in L)
				...
				if (C && istype(C, /list))
					...
					for (var/obj/machinery/camera/I in C)
						dat2 += text("[]<A HREF=byond://?[REF(src)];switchcamera=[REF(I)]>[]</A>", (dat2=="") ? "" : " | ", I.c_tag)
					dat += text("-- [] ([])", A.name, (dat2!="") ? dat2 : "No Camera")
				else if (C && istype(C, /obj/machinery/camera))
					var/obj/machinery/camera/Ctmp = C
					dat += text("-- [] (<A HREF=byond://?[REF(src)];switchcamera=[REF(C)]>[]</A>)", A.name, Ctmp.c_tag)
				else
...
/mob/living/silicon/ai/triggerAlarm(class, area/A, O, obj/alarmsource)
	...
	if (O)
		if (C?.can_use())
			queueAlarm("--- [class] alarm detected in [A.name]! (<A HREF=byond://?[REF(src)];switchcamera=[REF(C)]>[C.c_tag]</A>)", class)
		else if (CL?.len)
			var/foo = 0
			var/dat2 = ""
			for (var/obj/machinery/camera/I in CL)
				dat2 += text("[]<A HREF=byond://?[REF(src)];switchcamera=[REF(I)]>[]</A>", (!foo) ? "" : " | ", I.c_tag)	//I'm not fixing this shit...
				foo = 1

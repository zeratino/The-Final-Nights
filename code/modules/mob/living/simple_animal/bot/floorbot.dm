...
/mob/living/simple_animal/bot/floorbot/get_controls(mob/user)
	...
	dat += "Special tiles: "
	if(tilestack)
		dat += "<A href='byond://?src=[REF(src)];operation=eject'>Loaded \[[tilestack.amount]/[maxtiles]\]</a><BR>"
	else
		dat += "None Loaded<BR>"
	...
	if(!locked || issilicon(user) || isAdminGhostAI(user))
		...
		dat += "Traction Magnets: <A href='byond://?src=[REF(src)];operation=magnet'>[HAS_TRAIT_FROM(src, TRAIT_IMMOBILIZED, BUSY_FLOORBOT_TRAIT) ? "Engaged" : "Disengaged"]</A><BR>"
	...
	if(target)
		...
		if(path.len == 0)
			if(!isturf(target))
				...
				path = get_path_to(src, TL, TYPE_PROC_REF(/turf, Distance_cardinal), 0, 30, id=access_card,simulated_only = FALSE)
			else
				path = get_path_to(src, target, TYPE_PROC_REF(/turf, Distance_cardinal), 0, 30, id=access_card,simulated_only = FALSE)

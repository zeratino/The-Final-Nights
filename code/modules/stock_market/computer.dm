...
a.updated {
	...
	dat += "<b>Stock Transaction Log:</b> <a href='byond://?src=[REF(src)];show_logs=1'>Check</a><br>"
	...
	if (vmode == 0)
		...
		for (var/datum/stock/S in GLOB.stockExchange.stocks)
			...
			dat += "<a href='byond://?src=[REF(src)];archive=[REF(S)]' class='[news ? "updated" : "default"]'>(A)</a> <a href='byond://?src=[REF(src)];viewhistory=[REF(S)]'>(H)</a></td>"

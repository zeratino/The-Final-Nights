...
/obj/structure/spider/eggcluster/process(delta_time)
	...
	if(amount_grown >= 100 && !ghost_ready)
		notify_ghosts("[src] is ready to hatch!", null, enter_link="<a href=byond://?[REF(src)];activate=1>(Click to play)</a>", source=src, action=NOTIFY_ORBIT, ignore_key = POLL_IGNORE_SPIDER)
		ghost_ready = TRUE
